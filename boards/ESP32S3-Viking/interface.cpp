...
// Low-level SPI byte exchange for XPT2046
static uint8_t xpt_spi_byte(uint8_t b) {
    uint8_t result = 0;
    for (int i = 7; i >= 0; i--) {
        ...
        delayMicroseconds(1);
        result = (result << 1) | digitalRead(TFT_MISO);
    }
    return result;
}

// Read a 12-bit ADC value from XPT2046
static uint16_t xpt_read(uint8_t cmd) {
    xpt_spi_byte(cmd);
    uint8_t hi = xpt_spi_byte(0x00);
    uint8_t lo = xpt_spi_byte(0x00);
    return ((hi << 8) | lo) >> 3;  // 12-bit result
}

// Read touch using hardware SPI transaction
...
void _setup_gpio() {
    ...
    pinMode(TOUCH_IRQ, INPUT);     // T_IRQ is active LOW
