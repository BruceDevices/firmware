...
// scan/menu entry, i.e. before esp_wifi is touched at all). Measured on the
// cardputer: with only ~31 KB free the scan half-inits Wi-Fi and the later
// teardown crashes (LoadProhibited in ieee80211). A full Wi-Fi bring-up needs
// the whole driver (~70 KB) but the largest single contiguous request is what
// gates it; 40 KB refuses the "Wi-Fi after BLE" case cleanly while allowing
// Wi-Fi first (>= ~106 KB with SD, ~131 KB without).
constexpr size_t RADIO_WIFI_MIN_DMA_BLOCK = 40 * 1024;

// Minimum contiguous DMA block required before bringing the BLE stack up.
// Measured: BLE controller init SUCCEEDED at ~55 KB largest block, FAILED at
// ~31 KB (E BLE_INIT: Malloc failed / bad_alloc abort). 48 KB separates them.
constexpr size_t RADIO_BLE_MIN_DMA_BLOCK = 48 * 1024;
