// Pure-software AES-128 (ECB). Ported verbatim (arithmetic only) from
// Momentum's lib/subghz/protocols/aes_common.c. No Flipper runtime deps.
#include "rolling_aes.h"
#include <string.h>

static const uint8_t aes_sbox[256] = {
    };

static const uint8_t aes_sbox_inv[256] = {
    };

static const uint8_t aes_rcon[10] = {0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1b, 0x36};

static uint8_t gf_mul2(uint8_t x) { return ((x >> 7) * 0x1b) ^ (x << 1); }

static void aes_subbytes(uint8_t *state) {
    for (uint8_t i = 0; i < 16; i++) state[i] = aes_sbox[state[i]];
}
static void aes_subbytes_inv(uint8_t *state) {
    for (uint8_t i = 0; i < 16; i++) state[i] = aes_sbox_inv[state[i]];
}

static void aes_shiftrows(uint8_t *state) {
    uint8_t temp;
    temp = state[1]; state[1] = state[5]; state[5] = state[9]; state[9] = state[13]; state[13] = temp;
    temp = state[2]; state[2] = state[10]; state[10] = temp;
    temp = state[6]; state[6] = state[14]; state[14] = temp;
    temp = state[15]; state[15] = state[11]; state[11] = state[7]; state[7] = state[3]; state[3] = temp;
}
static void aes_shiftrows_inv(uint8_t *state) {
    uint8_t temp;
    temp = state[13]; state[13] = state[9]; state[9] = state[5]; state[5] = state[1]; state[1] = temp;
    temp = state[2]; state[2] = state[10]; state[10] = temp;
    temp = state[6]; state[6] = state[14]; state[14] = temp;
    temp = state[3]; state[3] = state[7]; state[7] = state[11]; state[11] = state[15]; state[15] = temp;
}

static void aes_mixcolumns(uint8_t *state) {
    for (uint8_t i = 0; i < 4; i++) {
        uint8_t a = state[i * 4], b = state[i * 4 + 1], c = state[i * 4 + 2], d = state[i * 4 + 3];
        uint8_t a2 = gf_mul2(a), b2 = gf_mul2(b), c2 = gf_mul2(c), d2 = gf_mul2(d);
        state[i * 4] = a2 ^ b2 ^ b ^ c ^ d;
        state[i * 4 + 1] = a ^ b2 ^ c2 ^ c ^ d;
        state[i * 4 + 2] = a ^ b ^ c2 ^ d2 ^ d;
        state[i * 4 + 3] = a2 ^ a ^ b ^ c ^ d2;
    }
}
static void aes_mixcolumns_inv(uint8_t *state) {
    for (uint8_t i = 0; i < 4; i++) {
        uint8_t a = state[i * 4], b = state[i * 4 + 1], c = state[i * 4 + 2], d = state[i * 4 + 3];
        uint8_t a2 = gf_mul2(a), a4 = gf_mul2(a2), a8 = gf_mul2(a4);
        uint8_t b2 = gf_mul2(b), b4 = gf_mul2(b2), b8 = gf_mul2(b4);
        uint8_t c2 = gf_mul2(c), c4 = gf_mul2(c2), c8 = gf_mul2(c4);
        uint8_t d2 = gf_mul2(d), d4 = gf_mul2(d2), d8 = gf_mul2(d4);
        state[i * 4] = (a8 ^ a4 ^ a2) ^ (b8 ^ b2 ^ b) ^ (c8 ^ c4 ^ c) ^ (d8 ^ d);
        state[i * 4 + 1] = (a8 ^ a) ^ (b8 ^ b4 ^ b2) ^ (c8 ^ c2 ^ c) ^ (d8 ^ d4 ^ d);
        state[i * 4 + 2] = (a8 ^ a4 ^ a) ^ (b8 ^ b) ^ (c8 ^ c4 ^ c2) ^ (d8 ^ d2 ^ d);
        state[i * 4 + 3] = (a8 ^ a2 ^ a) ^ (b8 ^ b4 ^ b) ^ (c8 ^ c) ^ (d8 ^ d4 ^ d2);
    }
}

static void aes_addroundkey(uint8_t *state, const uint8_t *round_key) {
    for (uint8_t i = 0; i < 16; i++) state[i] ^= round_key[i];
}

void aes_key_expansion(const uint8_t *key, uint8_t *round_keys) {
    for (uint8_t i = 0; i < 16; i++) round_keys[i] = key[i];
    for (uint8_t i = 4; i < 44; i++) {
        uint8_t prev = (i - 1) * 4;
        uint8_t b0 = round_keys[prev], b1 = round_keys[prev + 1];
        uint8_t b2 = round_keys[prev + 2], b3 = round_keys[prev + 3];
        if ((i % 4) == 0) {
            uint8_t n0 = aes_sbox[b1] ^ aes_rcon[(i / 4) - 1];
            uint8_t n1 = aes_sbox[b2];
            uint8_t n2 = aes_sbox[b3];
            uint8_t n3 = aes_sbox[b0];
            b0 = n0; b1 = n1; b2 = n2; b3 = n3;
        }
        uint8_t back = (i - 4) * 4;
        b0 ^= round_keys[back]; b1 ^= round_keys[back + 1];
        b2 ^= round_keys[back + 2]; b3 ^= round_keys[back + 3];
        uint8_t cur = i * 4;
        round_keys[cur] = b0; round_keys[cur + 1] = b1;
        round_keys[cur + 2] = b2; round_keys[cur + 3] = b3;
    }
}

void aes128_encrypt(const uint8_t *expanded_key, uint8_t *data) {
    uint8_t state[16];
    memcpy(state, data, 16);
    aes_addroundkey(state, &expanded_key[0]);
    for (uint8_t round = 1; round < 10; round++) {
        aes_subbytes(state);
        aes_shiftrows(state);
        aes_mixcolumns(state);
        aes_addroundkey(state, &expanded_key[round * 16]);
    }
    aes_subbytes(state);
    aes_shiftrows(state);
    aes_addroundkey(state, &expanded_key[160]);
    memcpy(data, state, 16);
}

void aes128_decrypt(const uint8_t *expanded_key, uint8_t *data) {
    uint8_t state[16];
    memcpy(state, data, 16);
    aes_addroundkey(state, &expanded_key[160]);
    for (uint8_t round = 9; round > 0; round--) {
        aes_shiftrows_inv(state);
        aes_subbytes_inv(state);
        aes_addroundkey(state, &expanded_key[round * 16]);
        aes_mixcolumns_inv(state);
    }
    aes_shiftrows_inv(state);
    aes_subbytes_inv(state);
    aes_addroundkey(state, &expanded_key[0]);
    memcpy(data, state, 16);
}

void reverse_bits_in_bytes(uint8_t *data, uint8_t len) {
    for (uint8_t i = 0; i < len; i++) {
        uint8_t byte = data[i];
        uint8_t step1 = ((byte & 0x55) << 1) | ((byte >> 1) & 0x55);
        uint8_t step2 = ((step1 & 0x33) << 2) | ((step1 >> 2) & 0x33);
        data[i] = ((step2 & 0x0F) << 4) | (step2 >> 4);
    }
}

void rolling_aes_ecb_encrypt(const uint8_t key[16], const uint8_t in[16], uint8_t out[16]) {
    uint8_t expanded[176];
    aes_key_expansion(key, expanded);
    memcpy(out, in, 16);
    aes128_encrypt(expanded, out);
}

void rolling_aes_ecb_decrypt(const uint8_t key[16], const uint8_t in[16], uint8_t out[16]) {
    uint8_t expanded[176];
    aes_key_expansion(key, expanded);
    memcpy(out, in, 16);
    aes128_decrypt(expanded, out);
}
